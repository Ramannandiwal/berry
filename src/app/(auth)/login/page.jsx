...
function Login() {
  ...
  const handleSubmit = async () => {
   ...
   const iemail= await localStorage.getItem('email')
   const ipass=  await localStorage.getItem('password')
   if(iemail===email&&ipass===password){
    router.push('/')
    setEmail("")
    setPassword("")
   }else{
    console.log("wrong passowrd or email")
    return
   }
  ...
  return (
    <div className=" bg-[#eef2f6] h-screen w-screen flex justify-center items-center ">
      <div className="  bg-[#ffffff] w-[28rem] rounded-xl m-8 flex items-center  justify-around flex-col">
        <div className=" mt-5">
          <svg
            width="92"
            height="32"
        ...
        <div className=" mt-5 text-center ">
          <div className=" text-[#009d89] font-bold text-2xl">
            Hi, Welcome Back
          </div>
        ...
        <div className=" flex flex-col items-center  mt-5 w-full">
          ...
          <div className=" w-full flex justify-center relative">
            ...
            >
              {showPassword ? (
                <FontAwesomeIcon onClick={()=>{setshowPassword(!showPassword)}} icon={faEye} /> 
              ) : (
                <FontAwesomeIcon onClick={()=>{setshowPassword(!showPassword)}}  icon={faEyeSlash} /> 
              )}
            </div>
          </div>
        </div>
        <div className=" flex w-full items-center justify-between">
          <div className="  flex items-center p-2">
            <input
              ...
              checked={checked}
              id="checked"
              className=" ml-3 form-radio text-[#009d89] border-[#009d89] h-5 w-5"
              style={{ accentColor: checked ? "#009d89" : "#009d89" }} // Set the color here
            />
          ...
          <Link
            className=" m-3 p-2 text-[#009d89] font-bold"
        ...
        <div className=" m-5">
          <Link className=" font-bold p-2" href={"/register"}>
